...
// if the device is in horizontal mode, otherwise create a DetailsActivity in portrait mode.
    public class AthleticsTitlesFragment extends ListFragment {
        ...
        public void onActivityCreated(Bundle savedInstanceState) {
            ...
            ArrayAdapter<String> connectArrayToListView = new ArrayAdapter<String>(
                    getActivity(),
                    android.R.layout.simple_list_item_activated_1, AthleticsInfo.TITLES);

            // Connect the ListView to the data
            setListAdapter(connectArrayToListView);

            // Check if the FrameLayout with the id details exists
            View detailsFrame = getActivity().findViewById(R.id.athletics_details);
        ...
        void showDetails(int index) {
            ...
            if (mDualPane) {
                ...
                // details data
                AthleticsDetailsFragment athleticsDetails = (AthleticsDetailsFragment)
                        getFragmentManager().findFragmentById(R.id.athletics_details);
                ...
                if (athleticsDetails == null || athleticsDetails.getShownIndex() != index) {

                    // Make the DetailsFragment and give it the currently selected title index
                    athleticsDetails = AthleticsDetailsFragment.newInstance(index);
                    ...
                    ft.replace(R.id.athletics_details, athleticsDetails);
                ...
                intent.setClass(getActivity(), AthleticsDetailsActivity.class);
