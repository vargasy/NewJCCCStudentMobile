...
public class DiscountTitlesFragment extends ListFragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        ArrayAdapter<String> connectArrayToListView = new ArrayAdapter<String>(
                getActivity(),
                android.R.layout.simple_list_item_activated_1, DiscountInfo.TITLES);

        // Connect the ListView to the data
        setListAdapter(connectArrayToListView);

        // Check if the FrameLayout with the id details exists
        View detailsFrame = getActivity().findViewById(R.id.discount_details);
    ...
    void showDetails(int index) {
        ...
        if (mDualPane) {
            ...
            // details data
           DiscountDetailsFragment discountDetails = (DiscountDetailsFragment)
                    getFragmentManager().findFragmentById(R.id.discount_details);
            ...
            if (discountDetails == null || discountDetails.getShownIndex() != index) {

                // Make the DetailsFragment and give it the currently selected title index
                discountDetails = DiscountDetailsFragment.newInstance(index);
                ...
                ft.replace(R.id.discount_details, discountDetails);
            ...
            intent.setClass(getActivity(), DiscountDetailsActivity.class);
